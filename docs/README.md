...
SparkSpeech 是一个 Windows 优先的个人语音输入工具，用全局快捷键触发录音，将豆包流式 ASR 的识别结果交给 OpenRouter 兼容模型整理，最后保存到本地历史，并复制到剪贴板，可选自动粘贴到当前光标位置。
...
- 平台：Windows
- ASR：火山引擎豆包流式语音识别
- 文本整理：OpenRouter 的 OpenAI-compatible API
