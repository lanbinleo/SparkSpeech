...
SparkSpeech 是一个简约、大方、开源的类闪电说智能语音输入法。按下全局快捷键开始说话，录音会先保存到本地，再交给豆包流式 ASR 转写，随后由 OpenRouter-compatible 模型整理成适合直接粘贴的文本。
...
SparkSpeech 目前固定使用豆包作为 ASR provider，并使用 OpenRouter-compatible API 做文本优化。模型页会把“语音识别模型”和“文本优化模型”分开管理，文本模型可以保存多个候选项，再选择当前使用的模型。

OpenRouter 设置支持 API Key、Base URL、系统代理开关和模型列表管理。
